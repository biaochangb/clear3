...
function parseCommit(org_commit, commit){
    ...
    try {
        ...
        commit.files = !org_commit.files ? [] : org_commit.files.map(function(f) {
            ...
            if (f.changes > 0) {
                s.changes += parseInt(f.changes);
                s.additions += parseInt(f.additions);
                s.deletions += parseInt(f.deletions);
            }
            else if(f.status) {
        ...
        return function () {
            if (login) {
                ...
                if (!ghcs.users.hasOwnProperty(login) || !ghcs.users[login].hasOwnProperty("repos")) {
                    ...
                    ldrTop.show();
                    JSONP(makeUrl("http://10.4.8.108/clear2/home/event/" + typeUser + "/type/" + login), function (req) {
